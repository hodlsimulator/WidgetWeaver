...
struct RainForecastSurfaceConfiguration {
    ...
    // Solid inner core (keeps the envelope data-accurate while allowing a large fuzzy shell)
    var fuzzSolidCoreEnabled: Bool = true
    var fuzzSolidCoreInsetBandMultiplier: Double = 0.55
    var fuzzSolidCoreInsetPeakFraction: Double = 0.12
    var fuzzSolidCoreMinInsetPixels: Double = 0.0
...
private extension RainForecastSurfaceConfiguration {
    ...
    mutating func applyWidgetPlaceholderBudgetGuardrails(
        ...
        // Inner solid core clamps.
        fuzzSolidCoreInsetBandMultiplier = max(0.0, min(fuzzSolidCoreInsetBandMultiplier, 8.0))
        fuzzSolidCoreInsetPeakFraction = max(0.0, min(fuzzSolidCoreInsetPeakFraction, 0.75))
        fuzzSolidCoreMinInsetPixels = max(0.0, min(fuzzSolidCoreMinInsetPixels, 48.0))
