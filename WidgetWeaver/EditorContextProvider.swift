...
private enum EditorFocusSnapshotNormaliserDiagnostics {
    ...
    private static func debugLabel(for composition: EditorSelectionComposition) -> String {
        ...
        case .known(let categories):
            return "known(\(categories.map(\.rawValue).joined(separator: ",")))"
        }
    }
}

private enum EditorContextProviderDiagnostics {
    static func maybeLogUnknownSelectionComposition(
        ...
        if context.selection == .multi,
           context.selectionCount != nil,
           context.selectionComposition == .unknown,
           focusSnapshot.selectionComposition == .unknown {
