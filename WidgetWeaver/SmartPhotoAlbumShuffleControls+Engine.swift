...
struct SmartPhotoShuffleRules: Hashable, Sendable {
    ...
    static let defaultRules = SmartPhotoShuffleRules(
        includeScreenshots: false,
        minimumPixelDimension: 800,
        sortOrder: .newestFirst
    )

    static let minimumPixelDimensionRange: ClosedRange<Int> = 200...4000

    func normalised() -> SmartPhotoShuffleRules {
        let minDim = max(Self.minimumPixelDimensionRange.lowerBound, min(minimumPixelDimension, Self.minimumPixelDimensionRange.upperBound))
...
enum SmartPhotoShuffleRulesStore {
    enum Keys {
        static let includeScreenshots = "widgetweaver.smartRules.includeScreenshots.v1"
        static let minPixelDimension = "widgetweaver.smartRules.minPixelDimension.v1"
        static let sortOrder = "widgetweaver.smartRules.sortOrder.v1"
    }

    static func load(defaults: UserDefaults = AppGroup.userDefaults) -> SmartPhotoShuffleRules {
        let include = defaults.bool(forKey: Keys.includeScreenshots)

        let rawMin = defaults.object(forKey: Keys.minPixelDimension) as? Int
        let minDim = rawMin ?? SmartPhotoShuffleRules.defaultRules.minimumPixelDimension

        let rawOrder = defaults.string(forKey: Keys.sortOrder) ?? SmartPhotoShuffleRules.defaultRules.sortOrder.rawValue
        let order = SmartPhotoShuffleSortOrder(rawValue: rawOrder) ?? SmartPhotoShuffleRules.defaultRules.sortOrder

        return SmartPhotoShuffleRules(
            includeScreenshots: include,
            minimumPixelDimension: minDim,
            sortOrder: order
        ).normalised()
    }

    static func resetToDefaults(defaults: UserDefaults = AppGroup.userDefaults) {
        let r = SmartPhotoShuffleRules.defaultRules
        defaults.set(r.includeScreenshots, forKey: Keys.includeScreenshots)
        defaults.set(r.minimumPixelDimension, forKey: Keys.minPixelDimension)
        defaults.set(r.sortOrder.rawValue, forKey: Keys.sortOrder)
...
enum SmartPhotoAlbumShuffleControlsEngine {
    ...
    static func fetchImageAssetIdentifiers(albumID: String) -> [String] {
        ...
        assets.enumerateObjects { asset, _, _ in
            if !rules.includeScreenshots, asset.mediaSubtypes.contains(.photoScreenshot) { return }

            let minDim = min(asset.pixelWidth, asset.pixelHeight)
            if minDim < rules.minimumPixelDimension { return }
