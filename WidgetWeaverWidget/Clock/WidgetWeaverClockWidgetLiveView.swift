...
struct WidgetWeaverClockWidgetLiveView: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            let startOfDay = cal.startOfDay(for: anchorDate)
            ...
            ZStack {
                ...
                ProgressView(timerInterval: startOfDay...endOfDay, countsDown: false) {
                    EmptyView()
                } currentValueLabel: {
                    EmptyView()
                }
                .progressViewStyle(
                    WWClockDayProgressStyle(
                        palette: palette,
                        layout: layout,
                        scale: displayScale,
...
// MARK: - Progress-driven clock motor

private struct WWClockDayProgressStyle: ProgressViewStyle {
    let palette: WidgetWeaverClockPalette
    ...
    func makeBody(configuration: Configuration) -> some View {
        ...
        let f = max(0.0, min(1.0, raw))

        let derivedDate = startOfDay.addingTimeInterval(dayDuration * f)
        let angles = WWClockMonotonicAngles(date: derivedDate)

        return WWClockDynamicOverlayView(
            palette: palette,
            layout: layout,
...
// MARK: - Dynamic overlay (kept light)
    ...
    var body: some View {
        ZStack {
            WidgetWeaverClockHandsView(
                ...
                secondLength: layout.secondLength,
                secondWidth: layout.secondWidth,
                secondTipSide: layout.secondTipSide,
                scale: scale
            )
