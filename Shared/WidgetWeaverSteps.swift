...
// MARK: - Analytics (pure, no HealthKit)
...
public final class WidgetWeaverStepsEngine: @unchecked Sendable {
    ...
    /// Fetches daily steps from the earliest step sample date all the way to "now" (inclusive),
    /// then caches it in the App Group so the in-app timeline can scroll back years.
        ...
        return try await withCheckedThrowingContinuation { cont in
            let query = HKStatisticsQuery(
                ...
                if let error = error as NSError? {
                    // HealthKit "No data available" is not an access problem.
                    if error.domain == HKErrorDomain, error.code == 11 {
