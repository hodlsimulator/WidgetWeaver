...
struct RainForecastSurfaceConfiguration: Hashable {
    ...
    // Legacy uncertainty shaping (still used as a secondary term).
    ...
    // MARK: - New: chance thresholding

    /// Chance at/above this trends smoother. Chance below trends fuzzy.
    var fuzzChanceThreshold: Double = 0.60

    /// Soft transition half-width around the threshold.
    /// Example: 0.12 means transition spans roughly [threshold-0.12, threshold+0.12].
    var fuzzChanceTransition: Double = 0.12

    /// Minimum fuzz strength even at high chance, to keep the surface “made of fuzz”.
    var fuzzChanceMinStrength: Double = 0.22

    // MARK: - New: core edge erosion

    /// When enabled, a destinationOut raster mask erodes the core near the surface so fuzz defines the boundary.
    var fuzzErodeEnabled: Bool = true

    /// Overall erosion strength.
    var fuzzErodeStrength: Double = 0.65

    /// Higher values keep erosion tight to the surface (reduces deep cut-outs).
    var fuzzErodeEdgePower: Double = 2.25
    ...
    var canEnableFuzz: Bool {
        fuzzEnabled
        && fuzzMaxOpacity > 0.001
        && (fuzzWidthFraction > 0.001 || fuzzWidthPixelsClamp.upperBound > 0.1)
    }

    // MARK: - Hashable / Equatable (colour-safe)
    ...
    private static func colorKey(_ color: Color) -> UInt64 {
        ...
        if ui.getRed(&r, green: &g, blue: &b, alpha: &a) {
            let rr = UInt64(max(0, min(255, Int((r * 255.0).rounded()))))
            let gg = UInt64(max(0, min(255, Int((g * 255.0).rounded()))))
            let bb = UInt64(max(0, min(255, Int((b * 255.0).rounded()))))
            let aa = UInt64(max(0, min(255, Int((a * 255.0).rounded()))))
    ...
    static func == (lhs: RainForecastSurfaceConfiguration, rhs: RainForecastSurfaceConfiguration) -> Bool {
        ...
        && lhs.fuzzChanceThreshold == rhs.fuzzChanceThreshold
        && lhs.fuzzChanceTransition == rhs.fuzzChanceTransition
        && lhs.fuzzChanceMinStrength == rhs.fuzzChanceMinStrength

        && lhs.fuzzErodeEnabled == rhs.fuzzErodeEnabled
        && lhs.fuzzErodeStrength == rhs.fuzzErodeStrength
        && lhs.fuzzErodeEdgePower == rhs.fuzzErodeEdgePower
    ...
    func hash(into hasher: inout Hasher) {
        ...
        hasher.combine(fuzzChanceThreshold)
        hasher.combine(fuzzChanceTransition)
        hasher.combine(fuzzChanceMinStrength)

        hasher.combine(fuzzErodeEnabled)
        hasher.combine(fuzzErodeStrength)
        hasher.combine(fuzzErodeEdgePower)
