...
//
//  Seamless noise tiles used for nowcast dissipation textures.
//  Intended for use with GraphicsContext.Shading.tiledImage.
//
...
enum RainSurfaceSeamlessNoiseTile {
    ...
    // Authored tile size (pixels). Used as the “repeat basis” for scale calculations.
    ...
    private static func makeTile(
        ...
        // Sample in [0, 1) so there are no duplicated edge texels.
        let denomX = Double(w)
        let denomY = Double(h)
