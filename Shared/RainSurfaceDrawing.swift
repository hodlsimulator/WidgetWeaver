...
enum RainSurfaceDrawing {
    ...
    // MARK: - Fuzz (granular, outside-only, certainty-driven morph)
    ...
    private static func drawFuzz(
        ...
        let uncertaintyExponent = RainSurfaceMath.clamp(configuration.fuzzUncertaintyExponent, min: 0.35, max: 5.0)
        ...
        var fuzzStrengthByX = [Double](repeating: 0.0, count: w)
        ...
        for x in 0..<w {
            ...
            let c = RainSurfaceMath.clamp01(cRaw)

            certaintyByX[x] = c

            let u = RainSurfaceMath.clamp01(uncertaintyFloor + (1.0 - uncertaintyFloor) * (1.0 - c))
            uncertaintyByX[x] = u

            let s = pow(u, uncertaintyExponent)
            fuzzStrengthByX[x] = s
            if s > maxFuzzStrength { maxFuzzStrength = s }
            ...
            let widthStrength = 0.25 + 0.75 * s
            effectiveWidthByX[x] = fuzzWidthPx * widthFactor * widthStrength
        }

        if maxFuzzStrength <= 0.0008 { return }

        let cell = max(6, Int(round(configuration.fuzzClumpCellPixels)))
        let gridW = (w / cell) + 3
        let gridH = (yMax / cell) + 3
        var clumpGrid = [Double](repeating: 0.0, count: max(0, gridW * gridH))
        ...
        for y in 0..<yMax {
            for x in 0..<w {
                ...
                let strength = fuzzStrengthByX[x]
                if strength <= 0.0005 { continue }

                let densityScale = strength
                let alphaScale = minAlphaScale + (1.0 - minAlphaScale) * strength

                let cl = sampleClump(x: x, y: y)
                let clumpAmp = 0.62 + 0.38 * cl

                let r = RainSurfacePRNG.hash2D01(x: x, y: y, seed: seedFine)
        ...
        let image = Image(decorative: cgImage, scale: rasterScale, orientation: .up)
    ...
    private static func rgbComponents(
        from color: Color,
        fallback: (r: UInt8, g: UInt8, b: UInt8)
    ...
    private static func chamferDistanceTransformOutside(
        ...
        for i in 0..<count {
            if mask[i] > insideThreshold { dist[i] = 0 }
        }
